# example usage: python3 kmeans_pp_AE.py 3 333 0.0 input_1_db_1.txt input_1_db_2.txt
...
if __name__ == "__main__":
    ...
    #
    #     # Make column min_index that holds the closest centroid for each data point
    #     df_copy = df.copy()
    #     df_copy["min_index"] = df_copy.apply(lambda row: np.argmin(
    #                                                       np.array(
    #                                                       [euclid_dist(row, centroids.iloc[i]) for i in range(centroids.shape[0])])), axis=1)
    #
    #     sorted = df_copy.sort_values(by="min_index")
    #     new_centroids = pd.DataFrame([])
    #     for k in range(K):
    #         cluster_vals = sorted[sorted["min_index"] == k]  # k is centroid index
    #         new_centr = np.mean(cluster_vals, axis=0).drop(labels=["min_index"])
    #         new_centr = pd.DataFrame([new_centr])
    #         new_centroids = pd.concat([new_centroids, new_centr], ignore_index=True)
    #
    #     diffs = np.linalg.norm(centroids - new_centroids, axis=0)
    #     max_diff = np.max(diffs)
    #
    #     centroids = new_centroids
    #     curr_iter += 1
    #
    # print(f"Iterations until convergence: {curr_iter}")
    # print(centroids)
